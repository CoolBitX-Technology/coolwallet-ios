...
char* BTCBase58CheckCStringWithData(NSData* immutabledata) {
    ...
    /* 將NSData轉成hexString，再將該String的prefix，00fingerprint 改成 6Ffingerprint */
    ...
    /* 將改過的hexString傳回NSData */
    NSMutableData *newdata= [NSMutableData new];
    unsigned char whole_byte;
    char byte_chars[3] = {'\0','\0','\0'};
    int i = 0;
    NSUInteger length = newHexString.length;
    while (i < length-1) {
        char c = [newHexString characterAtIndex:i++];
        if (c < '0' || (c > '9' && c < 'a') || c > 'f')
            continue;
        byte_chars[0] = c;
        byte_chars[1] = [newHexString characterAtIndex:i++];
        whole_byte = strtol(byte_chars, NULL, 16);
        [newdata appendBytes:&whole_byte length:1];
    }
    
    /* 將改過prefix的data去做checksum */
    NSData* checksum = BTCHash256(newdata);
    [newdata appendBytes:checksum.bytes length:4];
    
    char* result = BTCBase58CStringWithData(newdata);
#else
